...
static void test_solve_solvable_3_3_puzzle()
{
    ...
    Board solvable(taquin_tokenise_board_string(solvable_puzzle), 3);
    ...
    assert(taquin_get_inversion_count(solvable.get_state(), 3) == 16);

    //Should have state hash 0x316082754
    assert(solvable.get_state_hash() == 0x316082754);

    //Should not currently be solved
    assert(!solvable.check_solved());
...
static void test_solve_solvable_4_4_puzzle()
{
    ...
    Board solvable(taquin_tokenise_board_string(solvable_puzzle), 4);

    //should be solvable
    assert(taquin_check_solvable(solvable_puzzle, 4));

    //Should have heuristic 39
    assert(solvable.get_heuristic() == 39);

    //should have 49 inversions
    assert(taquin_get_inversion_count(solvable.get_state(), 4) == 49);

    //Should have state hash 0x36d8f905e4b72a1c
    assert(solvable.get_state_hash() == 0x36d8f905e4b72a1c);
